...
C     PRINT 8000, RAT
 ...
 7400 FORMAT('LAM=',F10.6,3X,'MRR=',D10.4,3X,'MRI=',D10.4)
      DDELT=0.1D0*DDELT
      IF (DABS(RAT-1D0).LE.1D-6) THEN
        PRINT 8003, AXI
        VOLUME = 4./3.*P*AXI**3
        PRINT 8005,VOLUME
        BVAL = (VOLUME*3/(4*P*EPS**2))**(1.0D0/3)
        AVAL = EPS*BVAL
        PRINT 8006, 2*AVAL, 2*BVAL
        GEOMETRICAL_AREA = P*AVAL**2
        PRINT 8007,GEOMETRICAL_AREA
      ENDIF
 ...
 8005 FORMAT('VOLUME = ',F8.2)
 8006 FORMAT('2A,2B = ', 2F8.3)
 8007 FORMAT('MAX AREA = ', F8.3)
...
C     Computing scattering etc cross section
...
C  PHASE MATRIX [Eqs. (13)-(29) of Ref. 6]
     ...
     &          -S21*DCONJG(S21)+S22*DCONJG(S22))
      ...
      WRITE (6,*) CEXT, CSCA, WALB
      WRITE (6,*) CEXT/GEOMETRICAL_AREA, CSCA/GEOMETRICAL_AREA
